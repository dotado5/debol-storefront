    ...
    <aside className="w-60 sidebar" aria-label="Sidebar">
      <div className="overflow-y-auto py-4 px-3 shadow-md bg-[#f5f6f4] rounded dark:bg-gray-800 h-[462px] mr-[10px] sidebar">
        <ul className="space-y-2 grid grid-cols-2 medium:grid-cols-1">
          {collections.slice(0, 9).map((collection, index) => (
            ...
            >
              <LocalizedClientLink
                ...
                className=" items-center p-2 text-xs font-normal text-gray-900 rounded-lg dark:text-white hover:bg-white dark:hover:bg-gray-700 gap-2 category relative hidden medium:flex"
              >
          ...
          {other && (
            <ul className="pl-4 medium:grid-cols-1 w-60 absolute left-[15em] top-0 bg-[#f5f6f4] h-[59vh] z-[10000] subCategories hidden medium:grid ">
              {collections.slice(9, 18).map((collection, index) => (
                <LocalizedClientLink
                  href={`/collections/${collection.handle}`}
                  className=" items-center p-2 text-xs font-normal text-gray-900 rounded-lg dark:text-white hover:bg-white dark:hover:bg-gray-700 category relative hidden medium:flex w-44 gap-2"
                  key={index}
                >
                  {/* <img src={category.icon} alt="" className="w-4" /> */}
                  <Image
          ...
          <p
            className=" items-center p-1 text-xs font-normal text-gray-900 rounded-lg dark:text-white hover:bg-white dark:hover:bg-gray-700 gap-2 category relative hidden small:flex cursor-pointer"
            onClick={() => setOther(!other)}
          >
            <Image src={Icons[9]} alt={""} width={20} height={20} />
            <TranslationComponent query={"Other Collections"} />
          </p>
          <p className="flex items-center p-2 text-sm font-normal text-gray-900 rounded-lg dark:text-white hover:bg-white dark:hover:bg-gray-700 gap-2 category relative small:hidden medium:hidden">
            <Image src={Icons[9]} alt={""} width={20} height={20} />
            <TranslationComponent query={"Other Collections"} />
          </p>
        </ul>
